  ...
  <div class="addBrand-container">
    <div class="container">
      <el-form ref="ruleForm"
        ...
        <div>
          <el-form-item label="菜谱辣度:" prop="pungencyType">
            <el-select v-model="ruleForm.pungencyType"
                       placeholder="请选择菜谱辣度"
                       @change="$forceUpdate()">
              <el-option label="不辣" value="0" />
              <el-option label="微辣" value="1" />
              <el-option label="中辣" value="2" />
              <el-option label="重辣" value="3" />
            </el-select>
          </el-form-item>
          <el-form-item label="菜谱甜度:"
                        prop="sweetType">
            <el-select v-model="ruleForm.sweetType"
                       placeholder="请选择菜谱分类"
                       @change="$forceUpdate()">
              <el-option label="无糖" value="0" />
              <el-option label="少糖" value="1" />
              <el-option label="半糖" value="2" />
              <el-option label="多糖" value="3" />
              <el-option label="全糖" value="4" />
            </el-select>
          </el-form-item>
        </div>
        <div>
    ...
    return {
      name: {
        ...
        validator: (rule: any, value: string, callback: Function) => {
          if (!value) {
            callback(new Error('请输入菜谱名称'))
          } else {
            const reg = /^([A-Za-z0-9\u4e00-\u9fa5]){2,20}$/
            if (!reg.test(value)) {
              callback(new Error('菜谱名称输入不符，请输入2-20个字符'))
            } else {
              callback()
            }
          }
    ...
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        if (this.dishTable.length === 0) {
          return this.$message.error('菜谱下食材不能为空')
        }
        if (!this.ruleForm.image) return this.$message.error('菜谱图片不能为空')
