<template>
  <div class="dashboard-container">
    <div class="container">
      <div class="tableBar">
        <label style="margin-right: 10px">菜谱名称：</label>
        ...
        <label style="margin-right: 10px; margin-left: 20px">菜谱分类：</label>
...
export default class extends Vue {
  ...
  private statusHandle(row: any) {
    ...
    if (typeof row === 'string') {
      if (this.checkList.length == 0) {
        this.$message.error('批量操作，请先勾选操作食材！')
        return false
      }
    ...
    this.$confirm('确认更改该菜谱状态?', '提示', {
      ...
      setmealStatusByStatus(params)
        .then(res => {
          if (res.data.code === 1) {
            this.$message.success('菜谱状态已经更改成功！')
            this.init()
          } else {
            this.$message.error(res.data.msg)
          }
