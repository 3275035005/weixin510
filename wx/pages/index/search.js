		...
		search(){
			...
			searchName(that.name).then((res) => {
				...
				if(res.code === 1){
					...
					console.log(that.dishListData)
					console.log(res.data.setmeal.length)
					if(res.data.setmeal.length > 0){
						let setmealListData =
							res.data.setmeal &&
							res.data.setmeal.map((obj) => ({
								...obj,
								type: 2,
								newCardNumber: 0,
							}))
							
							that.dishListData = [...that.dishListData,...setmealListData]
					}
					console.log(that.dishListData)
				}
				that.setOrderNum()
